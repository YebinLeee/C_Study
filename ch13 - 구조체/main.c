...
int main(void) {
	...
	p1(); // p.570 구조체 멤버 참조 - 각 좌표 입력 받아 두 점 사이의 거리 구하기
	p2(); // p.572 구조체 안의 구조체 - 각 좌표 입력 받아 사각형의 면적과 둘레 구하기
	
	// programming #1-11, 13, 14
	
	p3(); // 구조체 참조 - 책
	p4(); // 구조체 안의 구조체 멤버 - account
	p5(); // 구조체 안의 구조체  - 메일
	p6(); // 구조체와 함수 매개변수 - 복소수
	p7(); // 구조체 포인터 매개변수 - 동일한 좌표인지 판별
	p8(); // 구조체와 함수 - 좌표 입력 받아 사분면 구하기
	p9(); // 구조체와 함수 - 원의 면적, 둘레 구하기
	p10(); // 구조체 배열 - 음식의 총 칼로리 구하기
	p11(); // 구조체 배열 - 2,30 대 직원의 정보 출력
	p12(); // 구조체 배열과 탐색 - 입력 받아 구조체 저장하고 탐색 대상 출력(strcmp 문자열 처리 함수)
	p13(); // typedef, union 공용체, enum - 해당하는 도형의 면적 구하기
	p14(); // 소규모의 음반 데이터베이스 만들기 - 음반 추가, 출력, 탐색, 삭제 (구조체 함수/문자열 처리 라이브러리 함수 응용)
